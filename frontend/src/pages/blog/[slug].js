...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://www.sense-ai.world";

// Facebook/OG 用のロケール対応（既存の OG_LOCALE_MAP を流用してOK）
const OG_LOCALE_MAP = {
  en: "en_US", ja: "ja_JP", de: "de_DE", fr: "fr_FR", "es-ES": "es_ES", "es-MX": "es_MX",
  ar: "ar_AR", id: "id_ID", ko: "ko_KR", nl: "nl_NL", "pt-BR": "pt_BR", "pt-PT": "pt_PT",
  sv: "sv_SE", da: "da_DK", no: "nb_NO", ms: "ms_MY", it: "it_IT", el: "el_GR",
  hr: "hr_HR", cs: "cs_CZ", sk: "sk_SK", th: "th_TH", tr: "tr_TR", uk: "uk_UA", // 必要に応じ追加
};

export default function BlogPost({ postId, mdx, front, alternates, canonical, ogLocale }) {
    ...
    <>
      <Head>
        ...
        {alternates.map(({ l, href }) => (
          <link key={l} rel="alternate" hrefLang={String(l).toLowerCase()} href={href} />
        ))}
        {/* x-default は任意（有無どちらでも可：Google関係者の見解）。含める場合は既定言語へ。 */}
        ...
        <meta property="og:image" content={`${SITE_URL}${front.ogImage || "/og-image.jpg"}`} />
        <meta property="og:locale" content={ogLocale} />
        {alternates
          .filter((a) => a.l !== locale && OG_LOCALE_MAP[a.l])
          .map((a) => <meta key={a.l} property="og:locale:alternate" content={OG_LOCALE_MAP[a.l]} />)}

        {/* JSON-LD（BlogPosting）— ページごとにローカライズ */}
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              author: front.authors?.map((a) => ({ "@type": "Organization", name: a.name })) ?? [{ "@type": "Organization", name: "Sense LLC" }],
              publisher: { "@type": "Organization", name: "Minutes.AI", logo: { "@type": "ImageObject", url: `${SITE_URL}/icon-master.png` } },
            }),
          }}
        />
      </Head>
