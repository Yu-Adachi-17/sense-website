...
/** =========================
 *  コメット風ネオン円（360°フェード）
 *  - コメットを全周に敷き詰め、ヘッド側が最も明るく
 *    反対側（テール）へ向かって徐々に透明になる表現
 * ========================= */
function NeonCircle({ size = 560, speed = 6, children, ariaLabel }) {
  // speed は未使用（静止表示）
  const W = size, H = size;
  const cx = W / 2, cy = H / 2;
  const r = size * 0.46;                       // stroke の中心半径
  ...
  // 全周を細かい弧に分割して、ヘッドに近いほど不透明＆やや太く
  const SEGMENTS = 1000;                         // 分割数（多いほど滑らか）
  const STEP = 360 / SEGMENTS;
  const ease = (t) => Math.pow(t, 2.0);        // フェードのカーブ

  const segments = Array.from({ length: SEGMENTS }, (_, i) => {
    const a0 = TAIL_START + i * STEP;
    const a1 = a0 + STEP;
    const t = (i + 1) / SEGMENTS;              // テール(0)→ヘッド(1)
    const alpha = 0.04 + 0.82 * ease(t);       // 透明度（テール→ヘッド）
    const w = strokeW * (0.52 + 0.32 * ease(t)); // 太さ（テール→ヘッド）
...
.neonCircle{
  ...
  height: auto;             /* ← 高さは aspect-ratio に任せる */
  display: grid;
  place-items: center;
  isolation: isolate;
  overflow: visible;        /* ← ここを clip から visible に */
}
.ringSvg{
  position: absolute;
  inset: 0;
...
:global(.pKicker){
  ...
  opacity: 0.85;
  font-size: clamp(22px, calc(var(--sz) * 0.07), 42px); /* ← 少し小さく */
  color: white; /* ← 白文字に変更 */
  text-shadow: 0 2px 6px rgba(255, 255, 255, 0.1); /* 軽い立体感 */
}

  :global(.pPrice .big){
    /* 例: 560pxの円で最大54px。vwではなく--sz基準に */
    font-size: clamp(28px, calc(var(--sz) * 0.10), 54px);
  }
  :global(.pPrice .unit){
  ...
  .neonInner{
    position: relative;
    z-index: 2;
    width: min(86%, calc(var(--sz) * 0.9)); /* ← 円に追従 */
    max-width: none;                        /* ← 固定max幅を解除 */
    text-align: left;                       /* ← 左詰め */
  }
      `}</style>
    </div>
    ...
    <figure className="calloutPie">
      ...
      >
        ...
        {(() => {
          ...
          const s = 2.4 + (4.8 - 2.4) * (v / sorted[0].value);
          const capR = s * 0.48;
          return (
          ...
          return [...left, ...right].map((it, i) => {
            ...
            return (
              <g key={`lbl-${i}`}>
                ...
                <text x={tx} y={ty + 18} textAnchor={anchor} dominantBaseline="hanging" style={{ fontWeight: 700, fontSize: 14, fill: "rgba(200,225,255,0.92)" }}>
                  {it.d.value}%
                </text>
              </g>
            );
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:description" content="Record your meeting and let AI produce clean, human-ready minutes—decisions and to-dos at a glance." />
        ...
        <section className="below">
          ...
          <section className="pricingSection" aria-labelledby="pricingHead">
            <div className="pricingHeadWrap">
              ...
              <p className="pricingSub">Simple, predictable pricing. Flexible plans for any workflow.</p>
            </div>

            <div className="pricingGrid">
              {/* 左：使い切り分数パック */}
              <NeonCircle size={560} ariaLabel="Prepaid minutes pricing">
                <div className="pCard">
                <div className="pKicker">prepaid</div>
                  ...
                  <ul className="pBullets">
                    <li>No expiry</li>
                    <li>Perfect for occasional meetings</li>
              ...
              {/* 右：サブスク */}
              <NeonCircle size={560} ariaLabel="Subscription pricing">
                <div className="pCard">
                  <div className="pKicker">Subscription</div>
                  <div className="pPrice">
                  ...
                  <ul className="pBullets">
                    ...
                    <li>Best for teams with regular meetings</li>
                    <li>Unlock all features on iOS</li>
                  </ul>
                </div>
              </NeonCircle>
            </div>

            <p className="pricingNote">Prices in USD. Taxes may apply by region. Auto-renew; cancel anytime.</p>
          </section>
      ...
      <style jsx>{`
        ...
        .heroTop {
          ...
          filter: drop-shadow(0 0 10px rgba(160, 145, 255, 0.35)) drop-shadow(0 0 2px rgba(130, 150, 255, 0.2));
          pointer-events: none;
        }
        .below {
          position: relative;
        ...
        .deviceGlass {
          ...
          overflow: hidden;
          background: linear-gradient(180deg, rgba(var(--glassA), 0.55) 0%, rgba(var(--glassB), 0.5) 100%);
        ...
        @keyframes shoot {
          ...
          60% {
            transform: rotate(var(--a)) translateX(calc(var(--emit-radius) * 0.66)) scale(0.9);
        ...
        .reflection {
          ...
          width: 200vmax;
          height: 40vh;
          background: radial-gradient(120vmin 60% at 50% 0%, rgba(140, 150, 255, 0.28) 0%, rgba(140, 150, 255, 0.1) 40%, transparent 75%);
          filter: blur(14px);
          opacity: 0.7;
        }
        ...
        .simplyRight {
          ...
          aspect-ratio: 16 / 11;
          overflow: hidden;
          background: linear-gradient(180deg, rgba(36, 48, 72, 0.55) 0%, rgba(56, 78, 96, 0.5) 100%);
  ...
  margin-bottom: clamp(8px, 4vh, 40px); 
      ...
      <style jsx>{`
        .pageFooter {
          ...
          background: linear-gradient(0deg, rgba(10, 14, 28, 0.6) 0%, rgba(10, 14, 28, 0.3) 100%);
          color: #eaf4f7;
        }
        .footInner {
          max-width: 1200px;
