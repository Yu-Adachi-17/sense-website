...
import { getDebugTranscript } from '../debug/debugTranscripts';
...
const shouldUseTextMode = () => {
  try {
    if (typeof window !== 'undefined') {
      ...
      // URL で /?debug=1 or /?text=1 のときだけ有効
      if (q.get('debug') === '1' || q.get('text') === '1') {
        return true;
      }

      // たとえば一度だけ devtools から
      // localStorage.setItem('force_text_mode', '1')
      // しておくと、クエリなしでもずっとテキストモード
      if (localStorage.getItem('force_text_mode') === '1') {
        return true;
      }
    }
  } catch {
