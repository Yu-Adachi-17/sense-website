...
function bulletsBaseFont(count) {
  switch (count) {
    case 1:
      return 58;
    case 2:
...
export default function ProposalPage({ slide, pageNo, isIntelMode, hasPrefetched, imageUrlByKey }) {
  ...
  useLayoutEffect(() => {
    ...
    const recompute = () => {
      ...
      const rowGapBase =
        bullets.length >= 5 ? 16 : bullets.length === 4 ? 18 : bullets.length === 3 ? 20 : 22;
      ...
      // ---- Bottom font ----
      const baseBulletsFont = bulletsBaseFont(bullets.length) * scale * 0.92;

      const bottomTarget = clamp(baseBulletsFont + 10 * scale, 52 * scale, 74 * scale);
      ...
      const bulletFont = bullets.length
        ? binarySearchMaxFont({
            low: minBullet,
              ...
              <div className="ppImageSquare">
                {resolvedSrc ? (
                  <img
                    ...
                    alt={cacheKey || slide?.title || "proposal"}
                    className="ppImg"
                  />
                ) : (
            ...
            <div className="ppBulletsCol">
              {bullets.length ? (
                <ul className="ppBullets">
                  {bullets.map((b, i) => (
                    <li key={`${slide?.id || "slide"}-proposal-${i}`} className="ppBulletRow">
